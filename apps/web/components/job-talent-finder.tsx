...
export function JobTalentFinder({
  ...
  const availableUploadSlots = MAXIMUM_CANDIDATES - populatedCandidates.length;
  ...
  function addExtractedCandidates(documents: DocumentExtraction[]) {
    setCandidates((current) => {
      ...
      for (const document of documents) {
        ...
        } else if (updated.length < MAXIMUM_CANDIDATES) {
  ...
  function addBlankCandidate() {
    if (candidates.length >= MAXIMUM_CANDIDATES) return;
    setCandidates((current) => [
      ...current,
  ...
  function submit(event: React.FormEvent) {
    ...
    for (const candidate of candidates) {
      if (!candidate.displayName.trim()) {
        ...
        errors[candidate.id] = "This resume duplicates another document in this job.";
      }
    }
    ...
    if (!scorecardApproved || Object.keys(errors).length || !privacyConfirmed) return;
    analysis.mutate();
  }
  ...
  return (
    <CollapsibleSection
      title="Find Talent"
      description="Upload one to five resumes and compare them against the approved scorecard."
      status={
        <span className="rounded-full border border-line px-2.5 py-1 text-[11px] font-semibold text-muted">
          {populatedCandidates.length}/{MAXIMUM_CANDIDATES} resumes
        </span>
      }
    >
      <div className="p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-end gap-2">
          <MultiResumeUpload
            availableSlots={availableUploadSlots}
      ...
      <form onSubmit={submit} className="mt-5" noValidate>
        <div className="grid gap-4 lg:grid-cols-2">
          {candidates.map((candidate, index) => (
            <article key={candidate.id} className="rounded-xl border border-line p-4">
              <div className="mb-4 flex items-start justify-between gap-3">
                <label className="min-w-0 flex-1 text-[10px] font-semibold uppercase tracking-wide text-muted">
                  ...
                  <input
                    ...
                    maxLength={100}
                    onChange={(event) =>
                      updateCandidate(candidate.id, { displayName: event.target.value })
                    }
              ...
              <DocumentInput
                ...
                onChange={(resume) => updateCandidate(candidate.id, { resume })}
                onExtractionChange={(extraction) => {
                  const update: Partial<CandidateDraft> = { extraction };
                  if (extraction && candidate.displayName.startsWith("Candidate ")) {
                    update.displayName =
                      extraction.raw_text.split("\n")[0]?.trim().slice(0, 100) ||
              ...
              {duplicateIds.has(candidate.id) && (
                <p role="alert" className="mt-2 text-xs font-medium text-amber">
        ...
        <div className="mt-5 grid gap-4 rounded-xl border border-line bg-canvas/40 p-4 lg:grid-cols-2">
          <label className="flex cursor-pointer items-start gap-3">
            ...
            />
            <span>
              <span className="block text-xs font-semibold text-ink">Blind-review display</span>
        ...
        {analysis.isPending && progress && (
          <p aria-live="polite" className="mt-3 text-xs font-medium text-brand-700">
            {progress.latest_event?.label ?? "Starting analysis…"} · {progress.completed_count}/
            {progress.candidate_count} complete
          </p>
        )}
      ...
      {analysis.data && (
        <TalentSearchSummary result={analysis.data} onViewEvidence={onViewEvidence} />
      )}
      </div>
    </CollapsibleSection>
  );
  ...
  return (
    <div className="mt-6 rounded-xl border border-brand-100 bg-brand-50 p-4">
      ...
      <p className="mt-1 text-xs leading-5 text-muted">
        Results are shown in upload order and have been added to this job’s analysis history.
      </p>
      <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {result.comparisons.map((item) => (
          <div key={item.candidate_id} className="rounded-lg border border-brand-100 bg-white p-3">
            <p className="text-sm font-semibold text-ink">{item.display_name}</p>
            <p className="mt-1 text-xs text-muted">
              Fit {Math.round(item.comparison.fit_score)} · Mandatory {resultLabel(item.comparison.mandatory_status)}
            </p>
            {onViewEvidence && (
              <button
                type="button"
